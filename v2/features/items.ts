...
function itemsListResolver({ node, key, data }: JsonSchemaResolverParams) {
...
export function parseItems(node: SchemaNode) {
    ...
    if (isObject(schema.items)) {
        ...
        node.itemsList = schema.items.map((itemSchema, index) =>
            node.compileSchema(itemSchema, `${spointer}/items/${index}`)
        );
...
export function itemsValidator({ schema, validators }: SchemaNode) {
    ...
    validators.push(
        ({ node, data, pointer = "#", path }: JsonSchemaValidatorParams): JsonError | JsonError[] | undefined => {
