import { createNode, isJsonError } from "./types";
/**
...
export default function getChildSchemaSelection(draft, property, schema = draft.rootSchema) {
    var _a;
    if (schema.oneOf) {
        return schema.oneOf.map((item) => draft.resolveRef(createNode(draft, item)).schema);
    }
    if ((_a = schema.items) === null || _a === void 0 ? void 0 : _a.oneOf) {
        return schema.items.oneOf.map((item) => draft.resolveRef(createNode(draft, item)).schema);
    }
