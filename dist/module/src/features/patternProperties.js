...
export const patternPropertiesFeature = {
...
function patternPropertyResolver({ node, key }) {
    var _a, _b;
    return (_b = (_a = node.patternProperties) === null || _a === void 0 ? void 0 : _a.find(({ pattern }) => {
...
function validatePatternProperties({ node, data, pointer = "#" }) {
    if (!isObject(data)) {
        return;
    }
    ...
    keys.forEach((key) => {
        ...
        if (matchingPatterns.length === 0 && schema.additionalProperties === false) {
            ...
            errors.push(node.errors.noAdditionalPropertiesError({
                key,
                pointer: `${pointer}/${key}`,
                schema,
                value,
