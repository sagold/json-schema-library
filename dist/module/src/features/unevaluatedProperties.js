...
export const unevaluatedPropertiesFeature = {
    ...
    for (let i = 0; i < unevaluated.length; i += 1) {
        ...
        if (isSchemaNode(child)) {
            if (child.validate(data[propertyName], `${pointer}/${propertyName}`, path).length > 0) {
                errors.push(node.errors.unevaluatedPropertyError({
        ...
        if (child === undefined) {
            if (node.if && isPropertyEvaluated(node.if, propertyName, data)) {
                ...
                const validationResult = node.unevaluatedProperties.validate(data[propertyName], `${pointer}/${propertyName}`, path);
