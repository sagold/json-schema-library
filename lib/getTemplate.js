 ...
 */
function getTemplate(schema, data, rootSchema = schema) {
    ...
    // eslint-disable-next-line no-use-before-define
    return TYPE[schema.type](schema, data, rootSchema);
}

const TYPE = {
    "string": (schema, data) => getDefault(schema, data, ""),
    "number": (schema, data) => getDefault(schema, data, 0),
    "boolean": (schema, data) => getDefault(schema, data, false),
    "object": (schema, data, rootSchema) => {
        const template = schema.default || {};
        data = data || {};
        if (schema.properties) {
            Object.keys(schema.properties).forEach((key) => {
                data[key] = getTemplate(schema.properties[key], data[key] || template[key], rootSchema);
            });
        } else if (schema.oneOf && (schema.oneOf.length === 1 || Object.keys(data).length === 0)) {
            // may throw if #/oneOf/*/type !== "object"
            return getTemplate(schema.oneOf[0], data, rootSchema);
        } else if (schema.oneOf) {
            const one = guessOneOfSchema(schema, data);
            if (one) {
                return getTemplate(one, data, rootSchema);
            }
            console.log("Failed resolving oneOf", schema.oneOf, data);
        }
        return data;
    },
    "array": (schema, data, rootSchema) => {
        const template = schema.default || [];
        data = data || [];
        schema.minItems = schema.minItems || 0;
        if (schema.items.oneOf && data.length === 0) {
            for (let i = 0; i < schema.minItems; i += 1) {
                data[i] = getTemplate(schema.items.oneOf[0], data[i] || template[i], rootSchema);
            }
            ...
            for (let i = 0; i < itemCount; i += 1) {
                ...
                const one = guessOneOfSchema(schema.items, value);
                if (one) {
                    data[i] = getTemplate(one, value, rootSchema);
                } else {
                    data[i] = value;
                }
            }
            ...
            for (let i = 0; i < schema.minItems; i += 1) {
                data[i] = getTemplate(schema.items, data[i] || template[i], rootSchema);
            }
        } else if (Array.isArray(schema.items)) {
            for (let i = 0, l = Math.min(schema.minItems, schema.items.length); i < l; i += 1) {
                data[i] = getTemplate(schema.items[i], data[i] || template[i]);
            }
        }
