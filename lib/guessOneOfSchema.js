...
function fuzzyObjectValue(one, data) {
    if (data == null) {
        return -1;
    }
    ...
    for (var i = 0; i < keys.length; i += 1) {
        const key = keys[i];
        if (data[key] != null && isValid(data[key], one.properties[key])) {
            value += 1;
        }
    }
    return value;
...
function guessOneOf(schema, data, rootSchema = schema) {
    for (let i = 0; i < schema.oneOf.length; i += 1) {
        const one = resolveRef(schema.oneOf[i], rootSchema);
        const schemaOfItem = isValid(data, one);
        if (schemaOfItem) {
            return schemaOfItem;
        }
    }
