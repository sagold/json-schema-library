...
for (const { key, file } of drafts) {
    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            const percentage = total === 0 ? 0 : Math.floor((passes / total) * 100);
            message = percentage === 0 ? "no tests" : `${percentage}% passed`;
            ...
            if (total === 0) {
                ...
                color = percentage >= 95 ? "green" : percentage >= 80 ? "yellow" : percentage >= 60 ? "orange" : "red";
            }
        } catch (e) {
